...
export default function CompanyPreview({ id }) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const res = await base44.entities.Company.filter({ id }, "-created_date", 1);
        const c = res?.[0] || null;
        if (!mounted) return;
        setCompany(c);
        setStatus(c?.status || "prospect");
        if (c?.id) {
          const jRes = await base44.entities.Job.filter({ company_id: c.id }, "-created_date", 5).catch(() => []);
  ...
  const updateStatus = async (val) => {
    ...
    setSaving(true);
    await base44.entities.Company.update(company.id, { status: val }).catch(() => {});
    setSaving(false);
    setStatus(val);
  };
  ...
  return (
    <div style={{ fontFamily: "-apple-system,BlinkMacSystemFont,'Helvetica Neue',Arial,sans-serif" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
              <h2 style={{ fontSize: 16, fontWeight: 700, color: "#1D1D1F", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{company.name}</h2>
            ...
            <div style={{ fontSize: 13, color: "#86868B" }}>{company.industry || "—"}</div>
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {company.location && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "#1D1D1F" }}>
              <MapPin style={{ width: 14, height: 14, color: "#86868B", flexShrink: 0 }} /> {company.location}
            </div>
          )}
          {company.website && (
          ...
          {primary?.email && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "#1D1D1F" }}>
              <Mail style={{ width: 14, height: 14, color: "#86868B", flexShrink: 0 }} /> {primary.email}
            </div>
          )}
          {primary?.phone && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "#1D1D1F" }}>
              <Phone style={{ width: 14, height: 14, color: "#86868B", flexShrink: 0 }} /> {primary.phone}
            </div>
          )}
          {primary?.name && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "#1D1D1F" }}>
      ...
      {(company.contacts || []).length > 1 && (
        <div style={{ padding: "14px 20px", borderBottom: "1px solid #F2F2F7" }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "#86868B", textTransform: "uppercase", letterSpacing: ".04em", marginBottom: 10 }}>All Contacts</div>
              ...
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#1D1D1F" }}>
                    {c.name || "—"} {c.is_primary && <span style={{ fontSize: 10, color: "#0071E3" }}>★</span>}
                  </div>
                  <div style={{ fontSize: 11.5, color: "#86868B" }}>{c.title || ""} {c.email ? `· ${c.email}` : ""}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {jobs.length > 0 && (
        <div style={{ padding: "14px 20px" }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "#86868B", textTransform: "uppercase", letterSpacing: ".04em", marginBottom: 10 }}>Active Roles</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {jobs.map(j => {
              const jb = { open:{bg:"rgba(48,161,78,.10)",c:"#16A34A"}, draft:{bg:"rgba(107,114,128,.10)",c:"#6B7280"}, on_hold:{bg:"rgba(245,158,11,.10)",c:"#D97706"}, filled:{bg:"rgba(59,130,246,.10)",c:"#2563EB"}, cancelled:{bg:"rgba(239,68,68,.10)",c:"#DC2626"} }[j.status] || {bg:"rgba(0,0,0,.05)",c:"#86868B"};
              return (
                <div key={j.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", borderRadius: 10, background: "#F9F9FB" }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 500, color: "#1D1D1F", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{j.title}</span>
      ...
      {company.description && (
        <div style={{ padding: "0 20px 20px" }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "#86868B", textTransform: "uppercase", letterSpacing: ".04em", marginBottom: 8 }}>About</div>
