-- ═══════════════════════════════════════════════════════════════════════════
-- 024_multitenancy.sql — Multi-tenant data isolation (workspace scoping)
--
-- Adapted from branch feat/multi-tenancy-p0-1's 012_multitenancy.sql so it
-- composes with current main: migration 020 already replaced the old
-- "authenticated_all" policies with approval-gated versions. This migration
-- therefore drops those approval-gated policies and recreates them as
-- workspace-scoped + approval-gated.
--
-- ⚠ MUST be applied to a PREVIEW Supabase and verified (two workspaces can't
--   see each other) BEFORE prod. See AUTH_SETUP.md §4 for the verification curl
--   pattern; run it twice, once per workspace.
--
-- ⚠ EDGE FUNCTIONS / BOTS use the service-role key, which BYPASSES RLS and does
--   NOT populate auth_workspace_id(). Any service-role INSERT into a tenant
--   table must set workspace_id explicitly, or it will fail the NOT NULL check.
--   (Tracked as the Edge-Function audit step of P0-1.)
-- ═══════════════════════════════════════════════════════════════════════════
BEGIN;

-- The single workspace that owns all pre-existing rows.
...
--    user_profiles is handled separately (its ws is set explicitly at signup,
--    so it gets NO stamp trigger — at signup auth_workspace_id() is still null).
-- ─────────────────────────────────────────────────────────────────────────────
DO $$
DECLARE
  t TEXT;
...
-- New signups (Register.jsx / AuthContext bootstrap) insert a profile WITHOUT
-- workspace_id — and they must: the row doesn't exist yet, so a stamp trigger
-- can't resolve one. Default every new profile into the default workspace;
-- multi-workspace provisioning can set it explicitly later.
ALTER TABLE user_profiles
...
-- ═════════════════════════════════════════════════════════════════════════════
-- 6. RLS policy swap — workspace + approval (020's gate) composed together.
-- ═════════════════════════════════════════════════════════════════════════════

-- 6a. Standard tenant tables: workspace-scoped AND approved.
DO $$
DECLARE
  t TEXT;
...
BEGIN
  FOREACH t IN ARRAY std_tables LOOP
    -- drop the 020 approval-gated policies (same name on every table)
    ...
    -- new workspace + approval policy
    EXECUTE format(
      'CREATE POLICY "workspace_all" ON %I
         USING (workspace_id = auth_workspace_id() AND auth_is_approved())
         WITH CHECK (workspace_id = auth_workspace_id() AND auth_is_approved())', t);
  END LOOP;
END $$;

-- 6b. Admin-gated tenant tables: workspace AND admin.
DO $$
DECLARE
  t TEXT;
...
CREATE POLICY "ws_read" ON ai_recruiter_settings
  FOR SELECT USING (workspace_id = auth_workspace_id() AND auth_is_approved());
...
CREATE POLICY "ws_insert" ON audit_logs
  FOR INSERT WITH CHECK (workspace_id = auth_workspace_id() AND auth_is_approved());
...
--      action; the brand-new user has no profile/workspace yet).
-- ─────────────────────────────────────────────────────────────────────────────
...
-- (a) At least one approved admin must survive.
SELECT email, role, status, is_locked, workspace_id
  FROM user_profiles
 WHERE role = 'admin' AND status = 'active' AND COALESCE(is_locked, FALSE) = FALSE;

-- (b) Every tenant table now carries a workspace_id NOT NULL column.
SELECT table_name
  FROM information_schema.columns
 WHERE column_name = 'workspace_id' AND is_nullable = 'NO'
 ORDER BY table_name;
...
--     return [] for candidates, not rows. Repeat with two different workspaces
--     to confirm cross-workspace isolation.
